...
public class DisplayManagerCollector extends Collector {
    ...
    private static String collectFlags(Display display) {
        ...
        try {
            ...
            SparseArray<String> flagsNames = new SparseArray<>();
            for (Field field : display.getClass().getFields()) {
                if (field.getName().startsWith("FLAG_")) {
                    flagsNames.put(field.getInt(null), field.getName());
                }
            }

            result.append(display.getDisplayId()).append(".flags=").append(activeFlags(flagsNames, flags))
                    .append('\n');
        } catch (SecurityException | NoSuchMethodException | IllegalArgumentException | IllegalAccessException | InvocationTargetException e) {
            Timber.e(e, e.getMessage());
        }
