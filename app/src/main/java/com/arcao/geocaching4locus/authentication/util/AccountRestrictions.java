...
public class AccountRestrictions {
    ...
    AccountRestrictions(Context context) {
        this.context = context.getApplicationContext();
        preferences = this.context.getSharedPreferences(PrefConstants.RESTRICTION_STORAGE_NAME, Context.MODE_PRIVATE);
        init();
    }
    ...
    private void init() {
        ...
        fullGeocacheLimitPeriod = preferences.getLong(PrefConstants.RESTRICTION__FULL_GEOCACHE_LIMIT_PERIOD, DEFAULT_FULL_GEOCACHE_LIMIT_PERIOD);
        renewFullGeocacheLimit = new Date(preferences.getLong(PrefConstants.RESTRICTION__RENEW_FULL_GEOCACHE_LIMIT, 0));
    }

    void applyRestrictions(boolean premium) {
        if (premium) {
