...
public class DownloadRectangleTask extends UserTask<Void, Integer, Intent> {
  ...
  protected Intent doInBackground(Void... params) throws Exception {
    ...
    try {
      ...
      int logCount = mPrefs.getInt(DOWNLOADING_COUNT_OF_LOGS, 5);

      boolean simpleCacheData = mPrefs.getBoolean(DOWNLOADING_SIMPLE_CACHE_DATA, false);
      if (simpleCacheData) {
  ...
  private List<Filter> createFilters() {
    ...
    boolean showDisabled = mPrefs.getBoolean(FILTER_SHOW_DISABLED, false);
    filters.add(new GeocacheExclusionsFilter(false, showDisabled ? null : true, null, null, null, null));

    boolean showFound = mPrefs.getBoolean(FILTER_SHOW_FOUND, false);
    if (!showFound) {
      filters.add(new NotFoundByUsersFilter(userName));
    }

    boolean showOwn = mPrefs.getBoolean(FILTER_SHOW_OWN, false);
    if (!showOwn) {
    ...
    if (premiumMember) {
      ...
      float difficultyMin = PreferenceUtil.getParsedFloat(mPrefs, FILTER_DIFFICULTY_MIN, 1);
      float difficultyMax = PreferenceUtil.getParsedFloat(mPrefs, FILTER_DIFFICULTY_MAX, 5);
      if (difficultyMin > 1 || difficultyMax < 5) {
        filters.add(new DifficultyFilter(difficultyMin, difficultyMax));
      }

      float terrainMin = PreferenceUtil.getParsedFloat(mPrefs, FILTER_TERRAIN_MIN, 1);
      float terrainMax = PreferenceUtil.getParsedFloat(mPrefs, FILTER_TERRAIN_MAX, 5);
  ...
  private GeocacheType[] getSelectedGeocacheTypes() {
    ...
    for (int i = 0; i < len; i++) {
      if (mPrefs.getBoolean(FILTER_CACHE_TYPE_PREFIX + i, true)) {
  ...
  private ContainerType[] getSelectedContainerTypes() {
    ...
    for (int i = 0; i < len; i++) {
      if (mPrefs.getBoolean(FILTER_CONTAINER_TYPE_PREFIX + i, true)) {
