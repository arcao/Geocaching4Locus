...
public @interface ReportsCrashes {
	 ...
	 * <ul>
	 * <li>{@link ReportingInteractionMode#TOAST} requires {@link #resToastText()}
	 * to be provided to define the text that you want to be displayed to the user
	 * when a report is being sent.</li>
	...
	/**
	 * Resource id for the user email address input label in the crash dialog. If
	 * not provided, disables the input field.
	 */
	 ...
	 * {@link ACRA#PREF_DISABLE_ACRA} or {@link ACRA#PREF_ENABLE_ACRA} preference.
	 * Default is to use the default SharedPreferences, as retrieved with
	 * {@link PreferenceManager#getDefaultSharedPreferences(Context)}.
	 */
	String sharedPreferencesName() default "";

	/**
	 * If using a custom {@link ReportsCrashes#sharedPreferencesName()}, pass here
	 * the mode that you need for the SharedPreference file creation:
	 ...
	 * activate alternative logcat buffers reporting. They will use the same other
	 ...
	 * This allows to get rid of the INTERNET permission. Reports content can be
	 * customized with {@link #reportFields()}. Default fields are:
	 ...
	 * not. Default is true. This is a change from versions of ACRA before 3.2 as
	 * in NOTIFICATION mode reports were previously kept until the user explicitly
	 * opens the Notification dialog AND choose to send or discard the report.
	 * Until then, on application restart, ACRA was issuing a new crash
	...
	/**
	 * ADDED BY H the ability to skip over certain shared pref fields
	 * 
	 * @return
	 */
	String[] omitSharedPrefs() default {};

	/**
	 * ADDED BY H the ability to skip over certain settings fields
	 * 
	 * @return
	 */
	String[] omitSettings() default {};
}
